...
void Poisson1DExt::setup()
{
    ...
    // Initialize the finite element space.
    {
        ...
        printf("  Quadrature points per cell = %d\n", quadrature->size());
    }
    printf("-----------------------------------------------\n");
    ...
    // Initialize the linear system.
    {
        ...
        // First, we need to initialize the sparsity pattern. This object
        // describes the structure of the system matrix (i.e. which entries
        // are nonzero). We use a DynamicSparsityPattern object to generate
        // the sparsity pattern, and then copy it into a SparsityPattern
        // object, which is more efficient for computations.
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
        ...
        for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                cell_rhs(i) += rhs_function.value(fe_values.quadrature_point(q)) *
...
void Poisson1DExt::solve()
{
    ...
    printf("  Linear system solved in %d CG iterations\n", solver_control.last_step());
}

void Poisson1DExt::output() const
{
    printf("===============================================\n");
